...
export default function AdminProjectDetail() {
  ...
  if (fl.loading) {
    return (
      <AppLayout
        pageHeader={{ title: "Loading...", icon: <Server size={18} /> }}
      >
  ...
  const memberData = useMemo(
    () =>
      members.map((m) => {
        ...
        const user =
          USE_MOCK ? MOCK_USERS.find((u) => u.id === m.userId) : null;
        return {
          ...m,
  ...
  const configFields = [
    {
      ...
      desc: "Number of federated nodes",
    },
    {
      label: "Byzantine Fraction",
      value: config.byzantineFraction,
      desc: "Proportion of adversarial nodes",
    },
    {
      label: "Attack Type",
      value: config.attackType.replace(/_/g, " "),
      desc: "Simulated attack vector",
    },
    {
      label: "Aggregation",
      value: config.aggregationMethod.replace(/_/g, " "),
      desc: "Robust aggregation strategy",
    },
    { label: "Rounds", value: config.numRounds, desc: "Total training rounds" },
    {
      label: "Dirichlet \u03b1",
      value: config.dirichletAlpha,
      desc: "Non-IID data distribution parameter",
    },
    {
      label: "Differential Privacy",
      value: config.useDifferentialPrivacy ? "Enabled" : "Disabled",
      desc: "DP noise injection for gradient updates",
    },
    {
      label: "Noise Multiplier",
      value: config.dpNoiseMultiplier,
      desc: "\u03c3 for DP Gaussian noise",
    },
    {
      label: "Clip Norm",
      value: config.dpMaxGradNorm,
      desc: "Max gradient L2 norm before clipping",
    },
    {
      label: "SABD Alpha",
      value: config.sabdAlpha,
      desc: "Sensitivity-Aware Byzantine Detection threshold",
    },
    {
      label: "Local Epochs",
      value: config.localEpochs,
      desc: "Training epochs per round per client",
    },
  ];
                    ...
                    >
                      ...
                      <DialogContent>
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            You will lose admin controls for this project. This
                    ...
                    <CardContent className="space-y-3 p-4">
                      ...
                      {isPrivate && currentInviteCode && (
                        <div className="flex items-center gap-3">
                          ...
                          <span className="mono-data text-lg font-bold tracking-widest">
                            {currentInviteCode}
                          </span>
                          <Button
                            size="sm"
                      ...
                      <div className="flex gap-2">
                        {!isPrivate && (
                          <ConfirmDialog
                            ...
                            onConfirm={() => {
                              ...
                              store.updateExtraProject(id, {
                                visibility: "private",
                                inviteCode: code,
                              });
                              toast.success(
                                `Project is now private. Code: ${code}`,
                              );
                            }}
                          />
                        )}
                        {isPrivate && (
                          <ConfirmDialog
                            trigger={
                              <Button size="sm" variant="outline">
                                <Globe size={12} className="mr-1" /> Make Public
                              </Button>
                            }
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {configFields.map((f) => (
                    ...
                    >
                      ...
                      <p className="mono-data mt-1 text-lg">
                        {String(f.value)}
                      </p>
