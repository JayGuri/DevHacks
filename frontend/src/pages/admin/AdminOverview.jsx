...
export default function AdminOverview() {
  ...
  const allNodes = useMemo(
    () => projects.flatMap((p) => nodesByProject[p.id] || []),
    [nodesByProject, projects],
  );

  const latestRounds = projects.map((p) => {
  ...
  const chartData = useMemo(() => {
    const maxLen = Math.max(
      ...projects.map((p) => (roundsByProject[p.id] || []).length),
      1,
    );
  ...
  return (
    <AppLayout title="System Overview">
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-5">
        ...
        />
        <StatCard label="Total ε" value={totalEpsilon.toFixed(2)} icon={Lock} />
      </div>
      ...
      {viewMode === "detailed" && (
        <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <StatCard
            label="Blocked Nodes"
            value={allNodes.filter((n) => n.isBlocked).length}
            icon={Server}
              ...
              <TableBody>
                {clusterNodes.slice(0, 30).map((n) => (
                  <TableRow key={`${n.projectId}-${n.nodeId}`}>
                    ...
                    <TableCell
                      className={`mono-data text-xs ${getTrustColor(n.trust)}`}
                    >
                      {formatPercent(n.trust * 100)}
                    </TableCell>
                    {viewMode === "detailed" && (
                      <TableCell
                        className={`mono-data text-xs ${n.cosineDistance > 0.45 ? "text-rose-500" : "text-muted-foreground"}`}
                      >
