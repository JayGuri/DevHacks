...
const SidebarLink = memo(({ icon: Icon, label, to, badge, end = false, onClick }) => {
  return (
    <NavLink
      to={to}
      end={end}
    ...
    >
      {({ isActive }) => (
        <>
          ...
          <Icon size={16} className={cn("shrink-0 transition-colors", isActive ? "text-primary" : "text-muted-foreground group-hover:text-foreground")} />
...
const SectionLabel = memo(({ children }) => (
  <div className="flex items-center gap-2 px-3 pb-1.5 pt-5">
    <span className="metric-label text-[9px] text-muted-foreground/50 whitespace-nowrap">{children}</span>
    <div className="h-px w-full bg-muted/30" />
  </div>
));

// ── Project nav item with expandable sub-items ──────────────────────
const ProjectNavItem = memo(({ project, currentUser, nodesByProject, onNavigate }) => {
...
export const SidebarContent = memo(({ onNavigate }) => {
  ...
  const managedProjectIds = isLead 
    ? getAllProjects({ extraProjects }).filter(p => p.createdBy === currentUser?.id).map(p => p.id)
    : [];
    
  const pendingRequestCount = joinRequests.filter(r => r.status === "pending" && managedProjectIds.includes(r.projectId)).length;

  const joinedProjectIds = userProjects[currentUser?.id] || [];
  const joinedProjects = getAllProjects({ extraProjects }).filter(p => joinedProjectIds.includes(p.id));
    ...
    <div className="flex h-full flex-col bg-card">
      ...
      <div className="relative flex flex-col px-6 py-8">
        <div className="flex items-baseline gap-1">
          <span className="font-display text-3xl font-extrabold tracking-tighter text-primary">ARFL</span>
      ...
      <nav className="flex-1 overflow-y-auto px-3 space-y-1">
        ...
        {isLead && (
          <>
            ...
            <SidebarLink icon={Monitor} label="System Overview" to="/admin/overview" />
            <SidebarLink icon={Layers} label="Manage Projects" to="/admin/projects" />
            <SidebarLink
              icon={UserPlus}
              label="Join Requests"
              to="/admin/requests"
            ...
            <SidebarLink icon={ShieldCheck} label="Security Panel" to="/admin/security" />
          </>
        )}
      </nav>
      ...
      <div className="mt-auto p-4">
        <div className="bg-muted/30 rounded-xl p-3 border border-border/40 flex items-center gap-3">
          ...
          <div className="flex-1 overflow-hidden">
            <p className="truncate text-sm font-bold tracking-tight">{currentUser?.name}</p>
