...
export default function TopNav({ breadcrumbs = [] }) {
  ...
  const myNotifications = notifications.filter((n) => {
    if (isLead) {
      if (n.type === "join_request" && managedProjectIds.includes(n.projectId))
        return true;
      if (["alert", "node_blocked", "config"].includes(n.type)) return true;
    }
    if (n.type === "request_approved" && n.targetUserId === currentUser?.id)
      return true;
    if (n.type === "request_rejected" && n.targetUserId === currentUser?.id)
      return true;
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-40 flex h-14 items-center justify-between border-b border-border bg-background/95 px-4 backdrop-blur-sm lg:left-64 lg:px-6">
          ...
          <PopoverContent align="end" className="w-80 p-0">
            ...
            <div className="border-t border-border px-4 py-2">
              <button
                className="text-xs text-primary hover:underline"
                onClick={() =>
                  navigate(isLead ? "/admin/requests" : "/dashboard/projects")
                }
              >
